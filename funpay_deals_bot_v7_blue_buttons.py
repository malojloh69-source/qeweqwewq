...
def blue_inline_button(*args, **kwargs) -> InlineKeyboardButton:
    """Create an inline button with Telegram's blue primary style by default."""
...
SCREEN_BANNERS = {
    ...
    "referrals": {
        "ru": Path(__file__).with_name("screen_referrals_ru.png"),
        "en": Path(__file__).with_name("screen_referrals_en.png"),
    },
}
...
def format_operation_time(value: object) -> str:
    if not value:
        return ""
    try:
        parsed = datetime.fromisoformat(str(value).replace("Z", "+00:00"))
...
def withdrawal_status_label(status: str) -> str:
    return {
        "created": "Создана",
...
def user_deal_metrics(user_id: int) -> dict[str, int]:
    metrics = {"sales": 0, "purchases": 0, "cancelled": 0, "withdrawals": 0}
    for deal in deals.values():
        ...
        if status == "completed":
            ...
            if deal.get("buyer_id") == user_id:
                metrics["purchases"] += 1
        elif status == "cancelled" and user_id in {deal.get("seller_id"), deal.get("buyer_id")}:
            metrics["cancelled"] += 1
    metrics["withdrawals"] = sum(
        1 for item in withdrawals.values() if item.get("user_id") == user_id
    )
    return metrics


def operation_history_text(user_id: int) -> str:
    records: list[tuple[float, str, str]] = []
    for deal_id, deal in deals.items():
        if user_id not in {deal.get("seller_id"), deal.get("buyer_id")}:
            continue
        role = "Продажа" if deal.get("seller_id") == user_id else "Покупка"
        status = deal_status_label(str(deal.get("status", "created")))
        amount = f"{deal.get('amount', '-')} {deal.get('currency', '')}".strip()
        timestamp = deal.get("updated_at") or deal.get("created_at")
        time_text = format_operation_time(timestamp)
        details = (
            f"• <b>Сделка {html.escape(str(deal_id))}</b> · {role}\n"
            f"  {html.escape(amount)} · {html.escape(status)}"
        )
        if time_text:
            details += f" · {time_text}"
        records.append((timestamp_sort_key(timestamp), str(deal_id), details))

    for withdrawal_id, item in withdrawals.items():
        if item.get("user_id") != user_id:
            continue
        status = withdrawal_status_label(str(item.get("status", "created")))
        amount = f"{item.get('amount', '-')} {item.get('currency', '')}".strip()
        timestamp = item.get("updated_at") or item.get("created_at")
        time_text = format_operation_time(timestamp)
        details = (
            f"• <b>Вывод {html.escape(str(withdrawal_id))}</b>\n"
            f"  {html.escape(amount)} · {html.escape(status)}"
        )
        if time_text:
            details += f" · {time_text}"
        records.append((timestamp_sort_key(timestamp), str(withdrawal_id), details))

    records.sort(key=lambda item: (item[0], item[1]), reverse=True)
    header = f'{premium_emoji("5341715473882955310", "📋")} <b>Последние операции</b>\n\n'
    if not records:
        return header + "История операций пуста."
    return header + "\n\n".join(record[2] for record in records[:8])


def format_currency_totals(values: dict[str, Decimal]) -> str:
    parts = [
        f"{format_decimal(amount)} {html.escape(currency)}"
...
def bot_statistics_text() -> str:
    active_statuses = {"created", "joined", "buyer_paid"}
    status_counts = {"active": 0, "completed": 0, "cancelled": 0}
    completed_volume: dict[str, Decimal] = {}
    for deal in deals.values():
        status = str(deal.get("status", "created"))
        if status in active_statuses:
            ...
            try:
                ...
                amount = Decimal("0")
            completed_volume[currency] = completed_volume.get(currency, Decimal("0")) + amount
    ...
    for item in withdrawals.values():
        ...
        withdrawal_counts[status] = withdrawal_counts.get(status, 0) + 1

    withdrawal_summary = ", ".join(
        f"{withdrawal_status_label(status)}: {count}"
        for status, count in sorted(withdrawal_counts.items())
    ) or "Нет данных"

    return (
        f'{premium_emoji("5341715473882955310", "📊")} <b>Статистика бота</b>\n\n'
        f'<b>Пользователей:</b> {len(profiles)}\n'
        f'<b>Всего сделок:</b> {len(deals)}\n'
        f'<b>Активных:</b> {status_counts["active"]}\n'
        f'<b>Завершённых:</b> {status_counts["completed"]}\n'
        f'<b>Отменённых:</b> {status_counts["cancelled"]}\n\n'
        f'<b>Заявки на вывод:</b> {html.escape(withdrawal_summary)}\n\n'
        f'<b>Объём завершённых сделок:</b>\n{format_currency_totals(completed_volume)}'
    )
...
def find_profile_by_username(value: str) -> tuple[int, dict] | None:
    target = normalize_username(value)
    if not target:
        return None
    for user_id, profile in profiles.items():
        username = str(profile.get("username", "")).strip().lstrip("@")
        if username and not username.isdigit() and username.casefold() == target:
...
EN_TRANSLATIONS = {
    ...
    "История операций пуста.": "Operation history is empty.",
    ...
    "Покупки": "Purchases",
    "Покупка": "Purchase",
    "Отменённые сделки": "Canceled deals",
    ...
    "Активных": "Active",
    "Отменённых": "Canceled",
    "Объём завершённых сделок": "Completed deal volume",
    "Нет данных": "No data",
    ...
    "Начисление баланса": "Adding balance",
    "Поиск пользователя": "Find user",
    "Карточка пользователя": "User profile",
    "Отправьте Telegram-юзернейм:": "Send the Telegram username:",
    "Пользователь не найден.": "User not found.",
    "Баланс начислен.": "Balance credited.",
    "Значение обновлено.": "Value updated.",
    "Юзер:": "Username:",
    "Рефералов:": "Referrals:",
    ...
    "Используйте:": "Use:",
    "Установка количества успешных сделок": "Setting successful deals count",
    "Отправьте:": "Send:",
    "ЮЗЕР": "USERNAME",
    "КОЛИЧЕСТВО": "COUNT",
...
def admin_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [blue_inline_button(text="Статистика бота", callback_data="admin_statistics")],
            [blue_inline_button(text="Начислить баланс", callback_data="admin_add_balance")],
            [blue_inline_button(text="Установить успешные сделки", callback_data="admin_set_success")],
            [blue_inline_button(text="Найти пользователя", callback_data="admin_find_user")],
            [back_button()],
        ]
    )
...
def profile_text(user) -> str:
    profile = get_profile(user.id, stored_username(user))
    metrics = user_deal_metrics(user.id)
    return (
        ...
        f'<b>Продажи:</b> {metrics["sales"]} · <b>Покупки:</b> {metrics["purchases"]}\n'
        f'<b>Отменённые сделки:</b> {metrics["cancelled"]}\n'
        f'<b>Заявки на вывод:</b> {metrics["withdrawals"]}'
    )
...
async def operation_history(callback: CallbackQuery, state: FSMContext) -> None:
    await answer_callback(callback)
    await state.clear()
    await edit_screen_message(
        callback.message,
        operation_history_text(callback.from_user.id),
    ...
    if not items:
        ...
        for wid, item in items:
            text += (
                ...
                f"Статус: {html.escape(withdrawal_status_label(str(item.get('status', 'created'))))}\n\n"
            )
...
async def amount_input(message: Message, state: FSMContext) -> None:
    ...
    await edit_state_message(
        ...
        InlineKeyboardMarkup(
            inline_keyboard=[
                [blue_inline_button(text="Ознакомлен(-а)", callback_data="confirm_warning")],
...
async def cancel_created_deal(callback: CallbackQuery) -> None:
    ...
    deal["cancelled_at"] = utc_now_iso()
    deal["updated_at"] = deal["cancelled_at"]
    save_persistent_data()
...
async def admin_statistics(callback: CallbackQuery, state: FSMContext) -> None:
    if not await require_test_admin(callback):
        return
    await answer_callback(callback)
    await state.clear()
...
async def admin_add_balance(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await edit_screen_message(
        ...
        "<code>@ЮЗЕР ВАЛЮТА СУММА</code>\n\n"
        "Пример: <code>@username RUB 1000.50</code>",
...
async def admin_balance_input(message: Message, state: FSMContext) -> None:
    ...
    if not profile.get("clezzy_access"):
        ...
        return
    parts = (message.text or "").split()
    currency = normalize_currency(parts[1]) if len(parts) == 3 else ""
    target = find_profile_by_username(parts[0]) if len(parts) == 3 else None
    valid = len(parts) == 3 and currency in SUPPORTED_CURRENCIES and target is not None
    amount = parse_amount(parts[2]) if valid else None
    if not valid or amount is None:
        error_text = "<b>Пользователь не найден.</b>" if len(parts) == 3 and target is None else (
            "<b>Неверный формат.</b>\n\n"
            "Используйте: <code>@ЮЗЕР ВАЛЮТА СУММА</code>"
        )
        await edit_state_message(
            message,
            state,
    ...
    target_username = str(target_profile.get("username", parts[0])).lstrip("@")
    add_balance(target_id, currency, amount)
    logging.info(
        "Balance changed admin_id=%s target_username=%s currency=%s amount=%s",
        message.from_user.id,
        target_username,
        currency,
        amount,
    ...
    await edit_state_message(
        ...
        f"Юзер: <b>@{html.escape(target_username)}</b>\n"
...
async def admin_set_success(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await edit_screen_message(
        callback.message,
        "<b>Установка количества успешных сделок</b>\n\n"
        "Отправьте: <code>@ЮЗЕР КОЛИЧЕСТВО</code>",
        reply_markup=text_input_back_keyboard("admin_panel"),
    )
...
async def admin_success_input(message: Message, state: FSMContext) -> None:
    ...
    if not profile.get("clezzy_access"):
        ...
        return
    parts = (message.text or "").split()
    target = find_profile_by_username(parts[0]) if len(parts) == 2 else None
    valid = len(parts) == 2 and target is not None and parts[1].isdigit()
    if not valid:
        error_text = "<b>Пользователь не найден.</b>" if len(parts) == 2 and target is None else (
            "<b>Неверный формат.</b>\n\n"
            "Используйте: <code>@ЮЗЕР КОЛИЧЕСТВО</code>"
        )
        await edit_state_message(
            message,
            state,
    ...
    save_persistent_data()
    logging.info(
        "Successful deals changed admin_id=%s target_username=%s count=%s",
        message.from_user.id,
        target_username,
        count,
    )
...
async def admin_find_user(callback: CallbackQuery, state: FSMContext) -> None:
    if not await require_test_admin(callback):
        return
    await answer_callback(callback)
    await state.set_state(TestAdmin.waiting_user_lookup)
    await state.update_data(
        ui_chat_id=callback.message.chat.id,
        ui_message_id=callback.message.message_id,
        ui_has_photo=bool(callback.message.photo),
    ...
    await edit_screen_message(
        callback.message,
        "<b>Поиск пользователя</b>\n\nОтправьте Telegram-юзернейм:\n<code>@username</code>",
        reply_markup=text_input_back_keyboard("admin_panel"),
    )


@router.message(TestAdmin.waiting_user_lookup)
async def admin_user_lookup_input(message: Message, state: FSMContext) -> None:
    profile = get_profile(message.from_user.id, stored_username(message.from_user))
    if not profile.get("clezzy_access"):
        await state.clear()
        await delete_user_message(message)
        return
    target = find_profile_by_username(message.text or "")
    if target is None:
        await edit_state_message(
            message,
            state,
        ...
        return
    _, target_profile = target
    target_username = str(target_profile.get("username", "")).lstrip("@")
    referrals = target_profile.get("referrals", [])
    referral_count = len({int(value) for value in referrals if str(value).isdigit()})
    await state.set_state(None)
    await edit_state_message(
        message,
        ...
        f"Юзер: <b>@{html.escape(target_username)}</b>\n"
        f"Баланс: {format_balance(target_profile)}\n"
        f"Успешные сделки: <b>{int(target_profile.get('successful_deals', 0))}</b>\n"
...
async def main() -> None:
    token = BOT_TOKEN or getpass("Введите новый BOT_TOKEN: ").strip()
    if not token:
        raise RuntimeError("BOT_TOKEN не может быть пустым.")
